...
export default defineSchema({
  ...
  buckets: defineTable({
    userId: v.id("users"),
    name: v.string(),
    bucketMode: v.optional(v.union(v.literal("spend"), v.literal("save"), v.literal("recurring"))), // Optional for migration
    ...
    lastContributionDate: v.optional(v.number()), // Last time contribution was applied
  ...
  expenses: defineTable({
    ...
    isAutoGenerated: v.optional(v.boolean()), // true if auto-created during rollover for recurring buckets
    // Calendar month this auto-generated row represents, e.g. "2026-05".
    // Set only on auto-generated rows. Used as the dedup key when the recurring
    // sync reconciles target state — we keep at most one auto-pay per
    // (bucketId, autoGeneratedForMonth).
    autoGeneratedForMonth: v.optional(v.string()),
    // True once a user has manually edited an auto-generated row. Sync stops
    // managing the row (no amount overwrites, no deletion) so the user's
    // intent wins. Sync also doesn't create a *replacement* auto-pay for the
    ...
    .index("by_bucket_auto_month", ["bucketId", "autoGeneratedForMonth"])
